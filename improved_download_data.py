...
BASE_LABEL_URL = "https://openwms.statkart.no/skwms1/wms.fkb4"
...
async def process_bbox(session, bbox, base_label_url, base_image_url, label_folder, image_folder, preferred_image_size, delay=2.0):
    """
    ...
    """
    label_url = get_url(base_label_url, ["veg", "bru"], bbox, preferred_image_size)
    image_url = get_url(base_image_url, ["ortofoto"], bbox, preferred_image_size)
    label_path = label_folder / f"label_{bbox[0]}_{bbox[1]}.png"
    image_path = image_folder / f"image_{bbox[0]}_{bbox[1]}.png"



    # Download and check the label, with a pause afterward
    if await download_and_check_label(session, label_url, label_path):
        await asyncio.sleep(delay)  # Pause after downloading the label
        image_bytes = await fetch_image(session, image_url)
        if image_bytes:
            await save_image(image_bytes, image_path)

async def process_bbox_sequentially(session, bbox, base_label_url, base_image_url, label_folder, image_folder, preferred_image_size, existing_image_filenames, delay=1.0):
    """
...
async def main():
    ...
    starting_point = [560732.1454, 7017453.3362]
    ending_point = [561504.6032, 7019458.9953]
    ...
    async with ClientSession() as session:
        for bbox in bboxes:
            await process_bbox_sequentially(session, bbox, base_label_url, base_image_url, label_folder, image_folder, preferred_image_size, existing_image_filenames, delay)
